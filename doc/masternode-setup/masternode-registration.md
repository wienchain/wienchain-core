...
WIEN Masternode Setup Guide ( Windows QT Wallet with Ubuntu Server )
=

## Prerequisites
- 25,000 WIEN
- WIEN latest QT Wallet on Windows Desktop ( https://github.com/wienchain/wienchain-core/releases )
- A Ubuntu Server ( Recommended: 4GB RAM, 2 cores vCPU )
  - Public IP with port 8888 opened from firewall
  - WIEN latest Linux Daemon on Ubuntu Server( https://github.com/wienchain/wienchain-core/releases )
  - WIEN latest Sentinel on Ubuntu Server( https://github.com/wienchain/wienchain-sentinel )

Steps
=

## 1. Prepare Wienchain-qt wallet
- Download the latest wienchain-qt wallet from https://github.com/wienchain/wienchain-core/releases
- Unzip the folder
- Fire up the wienchain-qt wallet
...
- Send 25,000 WIEN to the masternode address
...
- Send 0.0001 WIEN to the fee source address
- Wait for 15 confirmations
...
- `collateralHash`: The txid of the 25,000 WIEN collateral funding transaction
- `collateralIndex`: The output index of the 25,000 WIEN funding transaction
- `ipAndPort`: Masternode IP address and port, in the format x.x.x.x:yyyy (or provided by your hosting service)
- `ownerKeyAddr`: The new WIEN address generated above for the owner/voting address (myvotingaddress address)
- `operatorPubKey`: The BLS public key generated above (or provided by your hosting service)
- `votingKeyAddr`: The new WIEN address generated above, or the address of a delegate, used for proposal voting (myvotingaddress address)
- `operatorReward`: The percentage of the block reward allocated to the operator as payment in percentage ( 1 = 1% )
- `payoutAddress`: A new or existing WIEN address to receive the owner’s masternode rewards (any address)
...
```
{
  ...
  "collateralAddress": "WUcsT7oTKMYd6PthEy8rcJYJovUiQrwWYs",
  "signMessage": "WjwB84K6wx7naYNU9hm7WQbFz3uaW3C7g4|0|WRzGfcnotDbcKph836496NyGE7CvhrUseW|WRzGfcnotDbcKph836496NyGE7CvhrUseW|54e34b8b996839c32f91e28a9e5806ec5ba5a1dadcffe47719f5b808219acf84"
}
```
...
Now we will sign the content of the `signMessage` field using the private key for the collateral address as specified in `collateralAddress`. Note that no internet connection is required for this step, meaning that the wallet can remain disconnected from the internet in cold storage to sign the message. In this example we will again use Wienchain, but it is equally possible to use the signing function of a hardware wallet. The command takes the following syntax:
```
signmessage collateralAddress signMessage
```
Example:
```
signmessage WUcsT7oTKMYd6PthEy8rcJYJovUiQrwWYs "WjwB84K6wx7naYNU9hm7WQbFz3uaW3C7g4|0|WRzGfcnotDbcKph836496NyGE7CvhrUseW|WRzGfcnotDbcKph836496NyGE7CvhrUseW|54e34b8b996839c32f91e28a9e5806ec5ba5a1dadcffe47719f5b808219acf84"
```
Output:
```
...
# That's it!
Congratulations in setting up your first masternode! Feel free to discuss in https://wienchain.com if you need further assistance
