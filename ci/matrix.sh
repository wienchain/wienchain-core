...
export BUILDER_IMAGE_NAME="wienchain-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/wienchain"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_WIENCHAIN_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
