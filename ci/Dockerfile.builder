...
# wienchain_hash
RUN git clone https://github.com/wienchain/wienchain_hash
RUN cd wienchain_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} wienchain
RUN useradd -u ${USER_ID} -g wienchain -s /bin/bash -m -d /wienchain wienchain
...
RUN mkdir /wienchain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /wienchain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /wienchain-src
