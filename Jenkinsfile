...
// which are part of the Wienchain repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="wienchain-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /wienchain-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "wienchain-wienchain/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from wienchain-wienchain/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'wienchain-wienchain/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from wienchain-wienchain/develop"
            }
          }

          if (hasCache) {
            sh "cd /wienchain-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /wienchain-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /wienchain-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /wienchain-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /wienchain-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /wienchain-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /wienchain-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
