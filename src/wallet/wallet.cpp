...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                bool found = false;
                if (pcoin->tx->vout[i].nValue == 5741) continue; // do not use asset embedded
...
//            if (fDebug) LogPrint("selectcoins", "value %s confirms %d\n", FormatMoney(pcoin->vout[output.i].nValue), output.nDepth);
            ...
            if (pcoin->tx->vout[i].nValue == 5741) continue; // Avoid sending asset
...
bool CWallet::SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType nCoinType, bool fUseDirectSend) const
{
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if (out.tx->tx->vout[out.i].nValue == 5741) continue; // Avoid sending asset
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseDirectSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_DIRECTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("DirectSend doesn't support sending values that high yet. Transactions are currently limited to %1 WIEN."), sporkManager.GetSporkValue(SPORK_5_DIRECTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-wienchain-address
                        CScript scriptChange;
