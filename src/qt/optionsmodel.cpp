...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WIEN);
    ...
    if (!settings.contains("nExclusiveSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeWienchainAmount"))
            settings.setValue("nExclusiveSendAmount", DEFAULT_EXCLUSIVESEND_AMOUNT);
        else
            settings.setValue("nExclusiveSendAmount", settings.value("nAnonymizeWienchainAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in wienchain.cpp during shotdown to also remove the window positions
