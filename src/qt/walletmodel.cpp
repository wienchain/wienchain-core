...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered wienchain address / amount:
    ...
    if(recipients[0].fUseDirectSend && IsOldDirectSendEnabled() && total > sporkManager.GetSporkValue(SPORK_5_DIRECTSEND_MAX_VALUE)*COIN) {
        Q_EMIT message(tr("Send Coins"), tr("DirectSend doesn't support sending values that high yet. Transactions are currently limited to %1 WIEN.").arg(sporkManager.GetSporkValue(SPORK_5_DIRECTSEND_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal wienchain:URI (wienchain:XyZ...?message=example)
            {
