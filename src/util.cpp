...
const char * const BITCOIN_CONF_FILENAME = "wienchain.conf";
const char * const BITCOIN_PID_FILENAME = "wienchaind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "wienchain" is a composite category enabling all Wienchain-related debug output
                if(ptrCategory->count(std::string("wienchain"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Wienchain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Wienchain
    // Mac: ~/Library/Application Support/Wienchain
    // Unix: ~/.wienchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wienchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Wienchain";
#else
    // Unix
    return pathRet / ".wienchain";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty wienchain.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override wienchain.conf
