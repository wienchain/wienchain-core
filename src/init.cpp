...
void PrepareShutdown()
{
    ...
    RenameThread("wienchain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "wienchain (or specifically: chainlocks, gobject, directsend, keepass, llmq, llmq-dkg, llmq-sigs, masternode, mnpayments, mnsync, exclusivesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all Wienchain specific functionality (Masternodes, ExclusiveSend, DirectSend, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<wienchainaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
    ...
    strUsage += HelpMessageOpt("-exclusivesendamount=<n>", strprintf(_("Keep N WIEN anonymized (%u-%u, default: %u)"), MIN_EXCLUSIVESEND_AMOUNT, MAX_EXCLUSIVESEND_AMOUNT, DEFAULT_EXCLUSIVESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/wienchain/wienchain>";
    const std::string URL_WEBSITE = "<https://wienchain.com>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("wienchain-loadblk");
...
/** Sanity checks
 *  Ensure that Wienchain is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Wienchain process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // lite mode disables all Wienchain-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all Wienchain-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/wienchain/wienchain/pull/1817 and https://github.com/wienchain/wienchain/pull/1743
    ...
    // ********************************************************* Step 10d: schedule Wienchain-specific tasks
