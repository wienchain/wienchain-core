...
#if defined(NDEBUG)
# error "Wienchain cannot be compiled without assertions."
#endif
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Wienchain
...

    {
        ...
        if (nAbsurdFee && nFees > nAbsurdFee && nFees != 5000 * COIN)
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight >= 525600) {
        for (int i = 525600; i <= nPrevHeight; i += 525600) {
            nSubsidy -= nSubsidy * 0.06;
        }
    }
    LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, nSubsidy);

    // WIEN provide 30% for Governance
...
void ThreadScriptCheck() {
    RenameThread("wienchain-scriptch");
...
static int64_t nTimeWIENSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    */
    // WIEN: Wienchain released after BIP30
    ...
    // WIEN : CHECK TRANSACTIONS FOR DIRECTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (directsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(WIEN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("ConnectBlock(WIEN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(WIEN): spork is off, skipping transaction locking checks\n");
    }
    ...
    // WIEN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(WIEN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(WIEN): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(WIEN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeWIENSpecific += nTime5 - nTime4;
    LogPrint("bench", "    - WIEN specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeWIENSpecific * 0.000001);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if(Params().NetworkIDString() == CBaseChainParams::TESTNET || Params().NetworkIDString() == CBaseChainParams::REGTEST ) {
        ...
        if (nHeight > 129600 && block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams))
